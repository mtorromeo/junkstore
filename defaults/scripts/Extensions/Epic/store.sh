...
function Epic_getplatformconfig() {
    $EPICCONF --confjson "${1}" --platform Proton --fork "" --version "" --dbfile "$DBFILE"
}
...
function Epic_install() {
    ...
    $EPICCONF --addsteamclientid "${1}" "${2}" --dbfile "$DBFILE"
    TEMP=$($EPICCONF --update-umu-id "${1}" egs --dbfile "$DBFILE")
...
function Epic_uninstall() {
    ...
    $EPICCONF --clearsteamclientid "${1}" --dbfile "$DBFILE"
}

function Epic_getgamedetails() {
    IMAGE_PATH=""
    $EPICCONF --getgamedata "${1}" "${IMAGE_PATH}" --dbfile "$DBFILE" --forkname "Proton" --version "null" --platform "Windows"
    exit 0
}

function Epic_getgamesize() {
    $EPICCONF --get-game-size "${1}" "${2}"  --dbfile "$DBFILE"
}
...
function Epic_loginstatus() {
    ...
    $EPICCONF --getloginstatus --dbfile "$DBFILE" --dbfile "$DBFILE" "$OFFLINE_MODE"  $FLUSH_CACHE
}
...
function loginlaunchoptions() {
    ...
    Compatibility=$5
    CompatTooName=$6
    echo "{\"Type\": \"LaunchOptions\", \"Content\": {
        \"Exe\": \"${Exe}\",
        \"Options\": \"${Options}\",
        \"WorkingDir\": \"${WorkingDir}\",
        \"Name\": \"${Name}\",
...
function Epic_getsetting() {
    $EPICCONF --getsetting "$1" --dbfile "$DBFILE"
}
